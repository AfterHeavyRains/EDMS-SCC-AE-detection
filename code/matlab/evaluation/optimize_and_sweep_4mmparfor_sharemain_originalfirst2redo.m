...
%  function optimize_and_sweep_4mm()
clear
...
%% ===== Save results to CSV =====
writetable(To, fullfile(saveDir, 'StepO_results.csv'));
writetable(Tl, fullfile(saveDir, 'StepL_results.csv'));

save(fullfile(saveDir, 'best_params.mat'), 'bestO', 'bestL', 'params_O_final', 'params_L_final');
fprintf('\n✅ 已保存结果到: %s\n', saveDir);
...
txtFile = strrep(matFile, '.mat', '.txt');

% 1) 读取原始
if exist(matFile,'file')
    S = load(matFile);
    % 原始文件里建议有 signal / fs；其他命名可在此补充
    if isfield(S,'signal'), sig = S.signal; end
    if isempty(sig) && isfield(S,'sig'), sig = S.sig; end
    if isfield(S,'fs'), fs = S.fs; else, fs = 2e6; end
elseif exist(txtFile,'file')
    sig = load(txtFile);
    fs  = 2e6; % 若 txt 中没有 fs，就用默认 2 MHz；需要的话自己改
else
...
% 2) 预处理：带通 + 小波去噪
sig = sig(:);
sig = double(sig); % 先转 double 以防滤波/小波函数类型问题

try
    % 你的预处理管线：
    % signal = wavelet_denoise( bandpass_filter(signal,1e5,7e5,fs,6), 'sym4', 5, 0.3);
    sig_bp   = bandpass_filter(sig, 1e5, 7e5, fs, 6);
    sig_deno = wavelet_denoise(sig_bp, 'sym4', 5, 0.3);
    sig = sig_deno;
catch ME
    warning('Preprocess failed on data%d: %s. Fallback to raw.', i, ME.message);
    % 失败则退回原始
end
...
% WAVELET_DENOISE  稳健小波去噪（软阈值）
% - 自动限制分解层数到允许的最大层
% - 只对细节系数做阈值（近似系数保持）
% - 正确使用 L 的记账向量进行切片，避免越界

    if nargin < 2 || isempty(wavelet),         wavelet = 'sym4'; end
    if nargin < 3 || isempty(level),           level   = 5;      end
    if nargin < 4 || isempty(threshold_factor),threshold_factor = 0.3; end

    % 统一形状 & 数值类型
    x = double(signal(:)');      % 行向量更符合 wavedec 的内部实现

    % 边界处理：与小波工具箱默认一致，但显式设定更稳
    dwtmode('sym','nodisp');

    % 关键1：上限层数，避免“Skipping level …”/越界
    Lmax  = wmaxlev(numel(x), wavelet);
    if isempty(Lmax) || Lmax < 1, Lmax = 1; end
    level = min(level, Lmax);

    % 小波分解
    [C, L] = wavedec(x, level, wavelet);
    % L 的结构: [len(cA_level) len(cD_level) ... len(cD_1) len(x)]
    % C 的拼接:  cA_level | cD_level | ... | cD_1

    denoised_coeffs = C;

    % 预计算每段起止索引（注意不含 L(end)）
    cumL = cumsum([0, L(1:end-1)]);

    % 关键2：只遍历细节段 j=2 ... end-1
    for j = 2:numel(L)-1
        idx1  = cumL(j)   + 1;
        idx2  = cumL(j+1);
        coeff = C(idx1:idx2);

        % 估计噪声并计算阈值（通用 Donoho 规则 * 缩放因子）
        sigma = median(abs(coeff)) / 0.6745;
        thr   = threshold_factor * sigma * sqrt(2*log(numel(coeff)));

        % 软阈值
        denoised_coeffs(idx1:idx2) = wthresh(coeff, 's', thr);
    end

    % 重构（近似系数未动）
    y = waverec(denoised_coeffs, L, wavelet);

    denoised_signal = y(:);  % 返回列向量
end
