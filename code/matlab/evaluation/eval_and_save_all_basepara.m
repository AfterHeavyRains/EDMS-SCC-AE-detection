...
%     'stalta_mode','ratio','stalta_thresh',2.3,'env_smooth_ms',0.02, ... % ✅ ratio模式 
%     'snr_thresh', 9.0, ... 
%     'win_energy_pts', 200, 'bg_win_pts', 12000, ... 
%     'use_weak_scoring', true, ... % ✅ 若要启用分层评估，设 true 
%     'weak_quality_cut', 0.4, 'weak_min_sep_ms', 3.0, ... 
%     'min_dur_ms', 0.04, 'min_sep_ms', 0.3, ... 
%     'rate_k_weak', 0.4, ... 
%     'pre_denoised', true);
base = struct( ... 
    'use_sst', false, ... 
    'use_percentile_norm', true, 'entropy_pct_lo', 1, 'entropy_pct_hi', 99, 'rate_smooth_win', 5, ... 
    'use_fixed_rate', true, 'fixed_rate_thr', -1900, ... % ✅ 固定熵率阈模式 
    'tol_ms', 1.0, 'min_interval_ms', 2.6, ... 
    'use_energy_gates', true, 'energy_global_mult', 1.25, 'burst_thresh', 1.35, ... 
    'use_dynamic_bg', true, 'alpha', 3.0, ... 
    'stalta_mode','ratio','stalta_thresh',2.4,'env_smooth_ms',0.05, ... % ✅ ratio模式 
    'snr_thresh', 9.8, ... 
    'win_energy_pts', 200, 'bg_win_pts', 15000, ... 
    'use_weak_scoring', true, ... % ✅ 若要启用分层评估，设 true 
    'weak_quality_cut', 0.4, 'weak_min_sep_ms', 3.0, ... 
    'min_dur_ms', 0.07, 'min_sep_ms', 0.5, ... 
    'rate_k_weak', 0.4, ... 
    'pre_denoised', true);
if nargin>=1 && strcmpi(mode,'sweep')
    [TOP, ALL] = sweep_small(base);
...
params.entropy_thresh_weak = params.entropy_thresh + 0.24;
...
%% ======================== Sweep 小范围优化 ========================
function [TOP, ALL] = sweep_small(base)

%% ===== 路径与数据源（复用主程序设置） =====
rootBase = 'PATH\TO\full_dataset';   % <-- set to your local full-dataset root (see README)
mkData   = @(mm) fullfile(rootBase, sprintf('result%imm',mm), sprintf('result%imm',mm), 'data_denoised');
dataDirs = struct('mm2', mkData(2), 'mm4', mkData(4), 'mm6', mkData(6), 'mm8', mkData(8));
thOrder  = {'mm2','mm4','mm6','mm8'};

outDir = fullfile(rootBase, 'eval_exports', 'sweep');
if ~exist(outDir,'dir'), mkdir(outDir); end

%% ===== 基线参数（与 base 保持一致）=====
bestO.ent = struct('mm2',0.54,'mm4',0.54,'mm6',0.54,'mm8',0.54);
bestL.delta = 0.24;

%% ===== 搜索网格（精简优化）=====

G.rate_k_weak        = 0.4;  % 固定
G.weak_quality_cut   = 0.45; % 固定
G.stalta_thresh      = 2.3:0.1:3.0;        % 共8档
G.energy_global_mult = [1.15, 1.20, 1.25]; % 共3档
G.snr_thresh         = [9.0, 9.3, 9.6, 9.9]; % 共4档
G.min_sep_ms         = [0.4, 0.5, 0.6];    % 共3档
...
grid = allcomb(G.stalta_thresh, G.energy_global_mult, G.snr_thresh, G.min_sep_ms);
nC = size(grid,1);
...
% ===== 快速评估 =====
for ci = 1:nC
    fprintf('[%s] Sweep %d / %d ...\n', datestr(now,'HH:MM:SS'), ci, nC);

    p = base;  % ← 来自传入的 base
    p.fixed_rate_thr     = -1700;  % 灵敏 sweep 模式（推荐）
    p.stalta_thresh      = grid(ci,1);
    p.energy_global_mult = grid(ci,2);
    p.snr_thresh         = grid(ci,3);
    p.min_sep_ms         = grid(ci,4);
    ...
    for t = 1:numel(thOrder)
        ...
        sc = sc + 0.4*AGRo - 0.4*EORo + 0.6*AGRl - 0.4*EORl + 0.1*((CovO+CovL)/2);
    end

    quickRows = [quickRows; ci, grid(ci,:), sc, ...
        metrics.mm2, metrics.mm4, metrics.mm6, metrics.mm8];
end

Vnames = {'cid','stalta_thresh','energy_mult','snr_thresh','min_sep_ms','score', ...
...
fprintf('Saved quick sweep subset.\n');
...
for r = 1:K
    ...
    for t = 1:numel(thOrder)
        ...
        p.entropy_thresh = bestO.ent.(tk);
        p.entropy_thresh_weak = p.entropy_thresh + bestL.delta;
        [mAGRo, mAGRl, mEORo, mEORl, mCovO, mCovL, nOK] = eval_on_set(ddir, 1:200, p);
        met = [met, mAGRo, mAGRl, mEORo, mEORl, mCovO, mCovL, nOK];
    end

    sc = 0;
    for t = 1:numel(thOrder)
        b = 1+(t-1)*7;
        AGRo=met(b); AGRl=met(b+1); EORo=met(b+2); EORl=met(b+3); CovO=met(b+4); CovL=met(b+5);
        sc = sc + 0.4*AGRo - 0.4*EORo + 0.6*AGRl - 0.4*EORl + 0.1*((CovO+CovL)/2);
    end

    fullRows = [fullRows; table(row.cid,row.stalta_thresh,row.energy_mult,row.snr_thresh,row.min_sep_ms,sc, ...
        'VariableNames',{'cid','stalta_thresh','energy_mult','snr_thresh','min_sep_ms','score'}) ...
...
disp(TOP);run_ablation_study
fprintf('Saved full sweep top-K.\n');
end
...
baseP = struct( ...
    ...
    'weak_quality_cut', 0.45, 'weak_min_sep_ms', 3.0, ...
    'min_dur_ms', 0.04, 'min_sep_ms', 0.3, ...
    'rate_k_weak', 0.4, ...
    'pre_denoised', true);
...
ablations = {
 ...
 '- Entropy-rate (τ_R)',       @(p)setfield(p,'use_fixed_rate',false); % 去掉熵率门
 '- Tolerance match (Δt)',     @(p)setfield(p,'tol_ms',Inf);           % 不匹配
 '- Weak-event quality score', @(p)setfield(p,'use_weak_scoring',false);
 '- Local fine-sweep',         @(p)setfield(p,'weak_quality_cut',0.0);
 'Original baseline',          @(p)setfield(p,'use_weak_scoring',false);
...
p.entropy_thresh = 1.0;  % 熵全部关闭
p.use_fixed_rate = false;
end

function p = disable_envelope_branch(p)
p.stalta_thresh = 999;  % STA-LTA关掉
p.use_fixed_rate = false;
p.tol_ms = Inf;
end
