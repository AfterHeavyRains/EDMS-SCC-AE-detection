...
% 文件开头把签名改成：
% eval_and_save_all()
%    └── sweep_small()         ← 如果 mode='sweep'
%          ├── base_params()   ← 提供默认参数模板（初值集合）
%          └── eval_on_set()   ← 调用 run_detection_with_thresholds()
%                 └── run_detection_with_thresholds() ← 真正的检测执行逻辑
% 参数设置优先级： p =base_params()最高；在optimize_and_sweep_6mmparfor_sharemain_originalfirst，
% 优先级：fixed 段 ＜ defaults 段 ＜ base 段
...
if nargin>=1 && strcmpi(mode,'sweep_k')
    sweep_k_sensitivity();
    return;
end
...
%% ===== 你的“最佳参数”（来自日志）=====
% Original（基线）
bestO.ent = struct('mm2',0.520,'mm4',0.520,'mm6',0.520,'mm8',0.520);
bestO.tol = 1.80;
...
TauH = AGRo; TauR = AGRo; dT = AGRo;  % ⭐ 新增
...
%         [agro, agrl, eoro, eorl, covO, covL] = run_detection_with_thresholds(sig, fs, params);
        [agro, agrl, eoro, eorl, covO, covL, tau] = run_detection_with_thresholds(sig, fs, params);
    ...
    if SAVE_PER_THICK_CSV
        TT = table(idxList(:), repmat({Tnm},numel(idxList),1), ...
                    AGRo, AGRl, EORo, EORl, CovO, CovL, TauH, TauR, dT, okIds, ...
         'VariableNames',{'sample_id','thickness','AGRo','AGRl','EORo','EORl','CovO','CovL','TauH','TauR','dT','valid'});
...
parfor k = 1:numel(idxList)
    ...
    try
        [agro, agrl, eoro, eorl, covO, covL, tau] = run_detection_with_thresholds(sig, fs, params);
        AGRo_tmp(k)=agro; AGRl_tmp(k)=agrl;
        EORo_tmp(k)=eoro; EORl_tmp(k)=eorl;
        CovO_tmp(k)=covO; CovL_tmp(k)=covL;
        TauH(k)=tau.tauH; TauR(k)=tau.tauR; dT(k)=tau.dT;
    catch ME
        fprintf(2,'[WARN] #%d failed: %s\n', i, ME.message);
    end
end
...
function [AGR_orig, AGR_layered, EOR_orig, EOR_layered, Cov_orig, Cov_layered, tauInfo] = run_detection_with_thresholds(signal, fs, params)
...
if ~isfield(params,'energy_global_mult'), params.energy_global_mult = 1.1; end   % 1.15 全局能量倍率
...
% ---- 参数 ----
if ~isfield(params,'entropy_thresh'),      params.entropy_thresh = 0.52; end
if ~isfield(params,'rate_k'),              params.rate_k = 2.0; end
if ~isfield(params,'tol_ms'),              params.tol_ms = 0.3; end
if ~isfield(params,'entropy_thresh_weak'), params.entropy_thresh_weak = params.entropy_thresh + 0.24; end
if ~isfield(params,'rate_k_weak'),         params.rate_k_weak = 0.4; end
...
if ~isfield(params,'stalta_thresh'),       params.stalta_thresh     = 2.3;   end  % 3.2 ratio 模式用
...
if ~isfield(params,'min_dur_ms'),          params.min_dur_ms = 0.04; end
if ~isfield(params,'min_sep_ms'),          params.min_sep_ms = 0.30; end %0.5
% SNR 物理门（MAD）
if ~isfield(params,'snr_thresh'),          params.snr_thresh        = 9;    end %8.8
...
% ---- 新增：时间偏差分析 ----
tauH_all = []; tauR_all = []; dT_all = [];
% τ_H: HC vs 最近 envelope
tauH = nan;
if ~isempty(HC) && ~isempty(env_ms)
    tauH_all = zeros(numel(HC),1);
    for i=1:numel(HC)
        [~,j] = min(abs(env_ms - HC(i)));
        tauH_all(i) = env_ms(j) - HC(i);
    end
    tauH = mean(tauH_all,'omitnan');  % 平均时差 (ms)
end

% τ_R: LC vs 最近 envelope (可理解为残差信号延迟)
tauR = nan;
if ~isempty(LC) && ~isempty(env_ms)
    tauR_all = zeros(numel(LC),1);
    for i=1:numel(LC)
        [~,j] = min(abs(env_ms - LC(i)));
        tauR_all(i) = env_ms(j) - LC(i);
    end
    tauR = mean(tauR_all,'omitnan');
end

% Δt: Layered MC vs 最近 HC
dT = nan;
if ~isempty(MC) && ~isempty(HC)
    dT_all = zeros(numel(MC),1);
    for i=1:numel(MC)
        [~,j] = min(abs(HC - MC(i)));
        dT_all(i) = MC(i) - HC(j);
    end
    dT = mean(dT_all,'omitnan');
end

% ---- 导出结构体（作为第7个输出）----
tauInfo = struct('tauH',tauH, 'tauR',tauR, 'dT',dT, ...
                 'tauH_all',tauH_all, 'tauR_all',tauR_all, 'dT_all',dT_all);
...
% —— 搜索网格（小范围微调，可按需增减）——
G.tol_ms            = 1.80;
G.stalta_thresh     = [3.25, 3.35];
G.min_sep_ms        = [0.50, 0.60];
G.min_dur_ms        = [0.060, 0.065];
G.env_smooth_ms     = [0.020, 0.025];
G.energy_global_mult= [1.25, 1.3];
G.snr_thresh        = [9.8, 10.4];
% Layered 相关（目前表现很好，先少动；若想再涨 AGRl 可稍放开 cut）
G.weak_quality_cut  = 0.4;   % 可改为 [0.45, 0.44]
...
for ci = 1:nC
    ...
    quickRows = [quickRows; ...
        ci, grid(ci,:), sc, ...
        metrics.mm2, metrics.mm4, metrics.mm6, metrics.mm8];
...
Vnames = {'cid','tol_ms','stalta_thresh','min_sep_ms','min_dur_ms','env_smooth_ms','energy_mult','snr_thresh','weak_cut','rk_weak','score', ...
          ...
          'm4_AGRo','m4_AGRl','m4_EORo','m4_EORl','m4_CovO','m4_CovL','m4_n', ...
...
for r = 1:K
    ...
    fullRows = [fullRows; table( ...
        ...
        'm2_AGRo','m2_AGRl','m2_EORo','m2_EORl','m2_CovO','m2_CovL','m2_n', ...
         'm4_AGRo','m4_AGRl','m4_EORo','mbase_params4_EORl','m4_CovO','m4_CovL','m4_n', ...
        'm6_AGRo','m6_AGRl','m6_EORo','m6_EORl','m6_CovO','m6_CovL','m6_n', ...
...
p.energy_global_mult  = 1.25;
p.burst_thresh        = 1.3;
p.use_dynamic_bg      = true;
p.alpha               = 3.0;
p.use_weak_scoring    = false;
p.weak_quality_cut    = 0.4;
p.weak_min_sep_ms     = 3.0;

p.entropy_thresh      = 0.52;
p.rate_k              = 2.0;
p.tol_ms              = 1.0;
p.entropy_thresh_weak = p.entropy_thresh + 0.24;
p.rate_k_weak         = 0.4;
...
p.use_fixed_rate      = true;
p.fixed_rate_thr      = -1700;

p.stalta_mode         = 'ratio';
p.stalta_thresh       = 2.3;
p.stalta_quantile     = 0.9979;
p.env_smooth_ms       = 0.02;
p.min_dur_ms          = 0.06;
p.min_sep_ms          = 0.50;
...
% Shutdown parallel pool to prevent heap corruption in long sweeps
poolobj = gcp('nocreate');
if ~isempty(poolobj), delete(poolobj); end

% Best 2mm parameters from cross-validated grid search (opt_log_fine.csv)
% ent=0.39, tol=0.60, ent_w=0.61, rk_w=0.2
p0 = base_params();
p0.entropy_thresh      = 0.39;   % cross-validated best (not 0.42)
p0.tol_ms              = 0.60;   % cross-validated best
p0.entropy_thresh_weak = 0.61;   % ent_w from opt_log_fine
p0.rate_k_weak         = 0.2;    % rk_w from opt_log_fine
p0.use_fixed_rate      = false;  % use mean-k*std mode
p0.rate_smooth_win     = 5;
p0.stalta_mode         = 'quantile';
p0.stalta_quantile     = 0.9972;
p0.env_smooth_ms       = 0.02;
p0.min_dur_ms          = 0.052;
p0.min_sep_ms          = 0.45;
p0.snr_thresh          = 9.0;
p0.min_interval_ms     = 2.6;
p0.use_weak_scoring    = true;
p0.weak_quality_cut    = 0.40;
p0.weak_min_sep_ms     = 3.0;
p0.burst_thresh        = 1.35;
p0.alpha               = 3.0;

fprintf('\n=== k sensitivity sweep: 2mm, all 200 files ===\n');
fprintf(' k      AGR_l   EOR_l   Cov_l   n\n');
fprintf('%s\n', repmat('-',1,42));

results = zeros(numel(k_values), 3);
for i = 1:numel(k_values)
    k  = k_values(i);
    p  = p0; p.rate_k = k;
    [~,agrl,~,eorl,~,covl,nOK] = eval_on_set(dataDir, idxAll, p);
    results(i,:) = [agrl, eorl, covl];
    marker = '';
    if abs(k-3.5)<0.01, marker = '  <-- chosen'; end
    fprintf(' %.2f   %.3f   %.3f   %.3f   %d%s\n', k, agrl, eorl, covl, nOK, marker);
end

% Save
outFile = fullfile(rootBase, 'sigma_sweep_k_final.mat');
save(outFile, 'k_values', 'results');
fprintf('\n[Saved] %s\n', outFile);

% Stable region
mask = results(:,2) < 0.15 & results(:,3) > 0.85;
if any(mask)
    ks = k_values(mask);
    fprintf('Stable region (EOR<0.15 & Cov>0.85): k in [%.2f, %.2f]\n', min(ks), max(ks));
end
end
